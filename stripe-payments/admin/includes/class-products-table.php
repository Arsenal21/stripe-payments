...
class asp_list_table extends WP_List_Table {
	...
	public function get_search_results( $search ) {
		...
		$data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "posts WHERE `post_title` LIKE '%%%s%%' AND `post_type` = 'asp_products'", $search ), ARRAY_A );
