...
class ASP_PP_Handler {
	...
	public function showpp() {
		...
		if ( ! defined( 'WP_ASP_DEV_MODE' ) ) {
			...
			$a['scripts'][] = array(
				'footer' => true,
				'src'    => WP_ASP_PLUGIN_URL . '/public/assets/js/pp-handler-ss.js?ver=' . WP_ASP_PLUGIN_VERSION,
			);
		}
	...
	public function handle_confirm_pi() {
		...
		try {
			...
			$token = filter_input( INPUT_POST, 'source_id', FILTER_SANITIZE_STRING );
		...
		ASP_Debug_Logger::log( wp_json_encode( $intent ) );
